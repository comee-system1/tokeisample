<template>
	...
	>
		...
		<div class="tile-container">
			<div class="tile-header">線形ゲージ</div>
			<div class="buttons">
		...
		<div class="tile-content">
			<div style="width: 100%; padding: 0 1rem;">
				<div class="flex-row">
					<h4>売上: {{ data.items[data.items.length - 1].sales | format('c') }}</h4>
					<wj-linear-gauge
				...
				<div class="flex-row">
					<h4>費用: {{ data.items[data.items.length - 1].expenses | format('c') }}</h4>
					<wj-linear-gauge
				...
				<div class="flex-row">
					<h4>利益: {{ data.items[data.items.length - 1].profit | format('c') }}</h4>
					<wj-linear-gauge
						:value="data.items[data.items.length-1].expenses"
						:min="0"
						:max="1500"
				...
				<h3>{{ data.items[data.items.length - 1].date | format('Y') }}のKPI</h3>
			</div>
		</div>
	</div>
</template>
